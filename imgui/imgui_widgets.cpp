...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    const float wrap_limit_x = window->DC.TextWrapPos;
    const bool wrap_active = (wrap_limit_x >= 0.0f);

    if ((text_end - text <= 2000) || wrap_active)
    {
        const float wrap_width = wrap_active ? CalcWrapWidthForPos(window->DC.CursorPos, wrap_limit_x) : 0.0f;
        const ImVec2 text_dimensions = CalcTextSize(text, text_end, false, wrap_width);
        ImRect bounding_box(text_pos, text_pos + text_dimensions);
        ItemSize(text_dimensions, 0.0f);
        if (ItemAdd(bounding_box, 0))
        {
            RenderTextWrapped(bounding_box.Min, text, text_end, wrap_width);
        }
    }
    else
    {
        ...
        ImVec2 accumulated_text_size = { 0.0f, 0.0f };
        ImVec2 current_pos = text_pos;

        if (!g.LogEnabled)
        {
            const int lines_to_skip = static_cast<int>((window->ClipRect.Min.y - text_pos.y) / line_height);
            if (lines_to_skip > 0)
            {
                int skipped_lines = 0;
                while (line_ptr < text_end && skipped_lines < lines_to_skip)
                {
                    const char* line_end = static_cast<const char*>(memchr(line_ptr, '\n', text_end - line_ptr));
                    if (!line_end) line_end = text_end;
                    if (!(flags & ImGuiTextFlags_NoWidthForLargeClippedText))
                        accumulated_text_size.x = ImMax(accumulated_text_size.x, CalcTextSize(line_ptr, line_end).x);
                    line_ptr = line_end + 1;
                    ++skipped_lines;
                }
                current_pos.y += skipped_lines * line_height;
            }
        }

        if (line_ptr < text_end)
        {
            ImRect line_rect(current_pos, { FLT_MAX, line_height });
            while (line_ptr < text_end)
            {
                ...
                if (!line_end) line_end = text_end;
                accumulated_text_size.x = ImMax(accumulated_text_size.x, CalcTextSize(line_ptr, line_end).x);
                RenderText(current_pos, line_ptr, line_end, false);
                line_ptr = line_end + 1;
            ...
            current_pos.y += remaining_lines * line_height;
        }

        accumulated_text_size.y = current_pos.y - text_pos.y;
        ImRect bounding_box(text_pos, text_pos + accumulated_text_size);
        ItemSize(accumulated_text_size, 0.0f);
        ItemAdd(bounding_box, 0);
    }
}
...
void ImGui::TextColoredV(const ImVec4& col, const char* fmt, va_list args)
{
    ...
    if (fmt[0] == '%' && fmt[1] == 's' && fmt[2] == '\0')
        TextEx(va_arg(args, const char*), nullptr, ImGuiTextFlags_NoWidthForLargeClippedText);
    else
        TextV(fmt, args);
    PopStyleColor();
}
...
{
    ImGuiContext& g = *GImGui;
    if (g.CurrentWindow->DC.TextWrapPos < 0.0f)
...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args)
{
    ...
    const char* value_text_start = &g.TempBuffer[0];
    const char* value_text_end = value_text_start + ImFormatStringV(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), fmt, args);
    const ImVec2 value_size = CalcTextSize(value_text_start, value_text_end, false);
    const ImVec2 label_size = CalcTextSize(label, nullptr, true);

    const ImVec2 position = window->DC.CursorPos;
    const ImRect value_bb(position, position + ImVec2(width, value_size.y + style.FramePadding.y * 2));
    const ImRect total_bb(position, position + ImVec2(width + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), ImMax(value_size.y, label_size.y) + style.FramePadding.y * 2));
    ItemSize(total_bb, style.FramePadding.y);
    if (ItemAdd(total_bb, 0))
    {
        RenderTextClipped(value_bb.Min + style.FramePadding, value_bb.Max, value_text_start, value_text_end, &value_size, ImVec2(0.0f, 0.0f));
...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
    ...
    const char* text_end = text_start + ImFormatStringV(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), fmt, args);
    ImVec2 label_size = CalcTextSize(text_start, text_end, false);
    ImVec2 total_dimensions = ImVec2(g.FontSize + (label_size.x > 0.0f ? (label_size.x + style.FramePadding.x * 2) : 0.0f), label_size.y);
    ImVec2 cursor_pos = GetCurrentWindow()->DC.CursorPos;
    cursor_pos.y += GetCurrentWindow()->DC.CurrLineTextBaseOffset;
    ItemSize(total_dimensions, 0.0f);
    ImRect bounding_box(cursor_pos, cursor_pos + total_dimensions);
    if (!ItemAdd(bounding_box, 0)) return;

    RenderBullet(GetCurrentWindow()->DrawList, bounding_box.Min + ImVec2(style.FramePadding.x + g.FontSize * 0.5f, g.FontSize * 0.5f), GetColorU32(ImGuiCol_Text));
    RenderText(bounding_box.Min + ImVec2(g.FontSize + style.FramePadding.x * 2, 0.0f), text_start, text_end, false);
}
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    flags |= ((flags & ImGuiButtonFlags_MouseButtonMask_) == 0) ? ImGuiButtonFlags_MouseButtonDefault_ : 0;
    flags |= ((flags & ImGuiButtonFlags_PressedOnMask_) == 0) ? ImGuiButtonFlags_PressedOnDefault_ : 0;

    bool flatten_hovered = (flags & ImGuiButtonFlags_FlattenChildren) && g.HoveredWindow && g.HoveredWindow->RootWindow == window;
    if (flatten_hovered) g.HoveredWindow = window;

#ifdef IMGUI_ENABLE_TEST_ENGINE
    if (id != 0 && g.LastItemData.ID != id) IMGUI_TEST_ENGINE_ITEM_ADD(bb, id);
#endif

    bool pressed = false, hovered = ItemHoverable(bb, id);

    if (hovered && g.DragDropActive && g.DragDropPayload.SourceId == id && !(g.DragDropSourceFlags & ImGuiDragDropFlags_SourceNoDisableHover))
        hovered = false;

    if (g.DragDropActive && (flags & ImGuiButtonFlags_PressedOnDragDropHold) && !(g.DragDropSourceFlags & ImGuiDragDropFlags_SourceNoHoldToOpenOthers) && IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem))
    {
        hovered = true;
        SetHoveredID(id);
        if (g.HoveredIdTimer >= DRAGDROP_HOLD_TO_OPEN_TIMER && g.HoveredIdTimer - g.IO.DeltaTime <= DRAGDROP_HOLD_TO_OPEN_TIMER)
        {
            pressed = true;
    ...
    if (flatten_hovered) g.HoveredWindow = g.HoveredWindow;
    ...
    if (hovered)
    {
        ...
        {
            int mouse_button = (flags & ImGuiButtonFlags_MouseButtonLeft) && g.IO.MouseClicked[0] ? 0 :
                (flags & ImGuiButtonFlags_MouseButtonRight) && g.IO.MouseClicked[1] ? 1 :
                (flags & ImGuiButtonFlags_MouseButtonMiddle) && g.IO.MouseClicked[2] ? 2 : -1;

            if (mouse_button != -1 && g.ActiveId != id)
            {
                ...
                if ((flags & ImGuiButtonFlags_PressedOnClick) || ((flags & ImGuiButtonFlags_PressedOnDoubleClick) && g.IO.MouseClickedCount[mouse_button] == 2))
                {
                    pressed = true;
                    if (!(flags & ImGuiButtonFlags_NoHoldingActiveId)) SetActiveID(id, window);
                    if (!(flags & ImGuiButtonFlags_NoNavFocus)) SetFocusID(id, window);
                    g.ActiveIdMouseButton = mouse_button;
                    FocusWindow(window);
                }
            ...
            {
                int mouse_released_button = (flags & ImGuiButtonFlags_MouseButtonLeft) && g.IO.MouseReleased[0] ? 0 :
                    (flags & ImGuiButtonFlags_MouseButtonRight) && g.IO.MouseReleased[1] ? 1 :
                    (flags & ImGuiButtonFlags_MouseButtonMiddle) && g.IO.MouseReleased[2] ? 2 : -1;

                if (mouse_released_button != -1)
                {
                    if (!(flags & ImGuiButtonFlags_Repeat) || g.IO.MouseDownDurationPrev[mouse_released_button] < g.IO.KeyRepeatDelay)
                    {
                        pressed = true;
                        if (!(flags & ImGuiButtonFlags_NoNavFocus)) SetFocusID(id, window);
                    }
            ...
            if (g.ActiveId == id && (flags & ImGuiButtonFlags_Repeat) && g.IO.MouseDownDuration[g.ActiveIdMouseButton] > 0.0f && IsMouseClicked(g.ActiveIdMouseButton, true))
                pressed = true;
        }
    ...
    {
        if (g.NavActivateId == id || IsNavInputTest(ImGuiNavInput_Activate, (flags & ImGuiButtonFlags_Repeat) ? ImGuiNavReadMode_Repeat : ImGuiNavReadMode_Pressed))
        {
            pressed = true;
            SetActiveID(id, window);
    ...
    if (g.ActiveId == id)
    {
        if (g.ActiveIdSource == ImGuiInputSource_Mouse)
        {
            ...
            int active_mouse_button = g.ActiveIdMouseButton;
            IM_ASSERT(active_mouse_button >= 0 && active_mouse_button < ImGuiMouseButton_COUNT);
            if (g.IO.MouseDown[active_mouse_button])
                held = true;
            else
            {
                bool release_within = hovered && (flags & ImGuiButtonFlags_PressedOnClickRelease);
                bool release_anywhere = (flags & ImGuiButtonFlags_PressedOnClickReleaseAnywhere);
                if ((release_within || release_anywhere) && !g.DragDropActive)
                {
                    if (!((flags & ImGuiButtonFlags_PressedOnDoubleClick) && g.IO.MouseReleased[active_mouse_button] && g.IO.MouseClickedLastCount[active_mouse_button] == 2) &&
                        !((flags & ImGuiButtonFlags_Repeat) && g.IO.MouseDownDurationPrev[active_mouse_button] >= g.IO.KeyRepeatDelay))
                    {
                        pressed = true;
                    }
                }
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    float area_ratio = window->OuterRectClipped.GetArea() / bb.GetArea();
    if (area_ratio < 1.5f)
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
    ...
    ImS64 scroll_pos = static_cast<ImS64>(window->Scroll[axis]);
    ScrollbarEx(bb, id, axis, &scroll_pos, static_cast<ImS64>(size_avail), static_cast<ImS64>(size_contents), rounding_corners);
    window->Scroll[axis] = static_cast<float>(scroll_pos);
}
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    const float width_frame = bb_frame.GetWidth();
    const float height_frame = bb_frame.GetHeight();
    if (width_frame <= 0.0f || height_frame <= 0.0f)
        return false;

    float alpha = 1.0f;
    if (axis == ImGuiAxis_Y && height_frame < g.FontSize + g.Style.FramePadding.y * 2.0f)
        alpha = ImSaturate((height_frame - g.FontSize) / (g.Style.FramePadding.y * 2.0f));
    if (alpha <= 0.0f)
        return false;

    const ImGuiStyle& style = g.Style;
    const bool interact_allowed = (alpha >= 1.0f);

    ImRect bb = bb_frame;
    bb.Expand(ImVec2(-ImClamp(IM_FLOOR((width_frame - 2.0f) * 0.5f), 0.0f, 3.0f), -ImClamp(IM_FLOOR((height_frame - 2.0f) * 0.5f), 0.0f, 3.0f)));

    const float scrollbar_size_v = (axis == ImGuiAxis_X) ? bb.GetWidth() : bb.GetHeight();

    IM_ASSERT(ImMax(size_contents_v, size_avail_v) > 0.0f);
    const ImS64 window_size_v = ImMax(ImMax(size_contents_v, size_avail_v), static_cast<ImS64>(1));
    const float grab_h_pixels = ImClamp(scrollbar_size_v * (static_cast<float>(size_avail_v) / static_cast<float>(window_size_v)), style.GrabMinSize, scrollbar_size_v);
    ...
    const ImS64 max_scroll = ImMax(static_cast<ImS64>(1), size_contents_v - size_avail_v);
    float scroll_ratio = ImSaturate(static_cast<float>(*p_scroll_v) / static_cast<float>(max_scroll));
    float grab_v_norm = scroll_ratio * (scrollbar_size_v - grab_h_pixels) / scrollbar_size_v;

    if (held && interact_allowed && grab_h_norm < 1.0f)
    {
        const float pos_v_scrollbar = bb.Min[axis];
        const float pos_v_mouse = g.IO.MousePos[axis];

        const float clicked_v_norm = ImSaturate((pos_v_mouse - pos_v_scrollbar) / scrollbar_size_v);
        SetHoveredID(id);

        bool is_seeking = false;
        if (g.ActiveIdIsJustActivated)
        {
            is_seeking = (clicked_v_norm < grab_v_norm || clicked_v_norm > grab_v_norm + grab_h_norm);
            if (is_seeking)
        ...
        *p_scroll_v = static_cast<ImS64>(scroll_v_norm * max_scroll);

        scroll_ratio = ImSaturate(static_cast<float>(*p_scroll_v) / static_cast<float>(max_scroll));
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec2& padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    if (!window->SkipItems)
    {
        ...
        if (ItemAdd(bb, id))
        {
            ...
            ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
            RenderNavHighlight(bb, id);
            float rounding = ImClamp(static_cast<float>(ImMin(padding.x, padding.y)), 0.0f, GImGui->Style.FrameRounding);
            RenderFrame(bb.Min, bb.Max, col, true, rounding);
            if (bg_col.w > 0.0f)
...
{
    PushID(reinterpret_cast<void*>(intptr_t(user_texture_id)));
    ImGuiID id = GImGui->CurrentWindow->GetID("#image");
    PopID();

    ImVec2 padding = (frame_padding >= 0) ? ImVec2(static_cast<float>(frame_padding), static_cast<float>(frame_padding)) : GImGui->Style.FramePadding;
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (!window->SkipItems)
    {
        ...
        if (ItemAdd(total_bb, id))
        {
            ...
            if ((g.LastItemData.InFlags & ImGuiItemFlags_MixedValue) != 0)
            {
            ...
            {
                float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
                RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), GetColorU32(ImGuiCol_CheckMark), square_sz - pad * 2.0f);
            }

            ImVec2 label_pos(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y);
            if (g.LogEnabled)
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    if (!window->SkipItems)
    {
        ...
        if (ItemAdd(total_bb, id))
        {
            ...
            float radius = (square_sz - 1.0f) * 0.5f;
            ...
            window->DrawList->AddCircleFilled(center, radius, bg_col, 16);
            if (active)
            {
                float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
...
{
    if (RadioButton(label, *v == v_button))
    {
        *v = v_button;
        return true;
    }
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    if (!window->SkipItems)
    {
        ...
        if (ItemAdd(bb, 0))
        {
            ...
            ImVec2 fill_br(ImLerp(bb.Min.x, bb.Max.x, fraction), bb.Max.y);
...
void ImGui::Bullet()
{
    ...
    if (!window->SkipItems)
    {
        ImGuiContext& g = *GImGui;
        const ImGuiStyle& style = g.Style;
        float line_height = ImMax(ImMin(window->DC.CurrLineSize.y, g.FontSize + style.FramePadding.y * 2), g.FontSize);
        ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(g.FontSize, line_height));
        ItemSize(bb);
        if (!ItemAdd(bb, 0))
        {
...
void ImGui::Dummy(const ImVec2& size)
{
    if (!GImGui->CurrentWindow->SkipItems)
    {
        const ImRect bb(GImGui->CurrentWindow->DC.CursorPos, GImGui->CurrentWindow->DC.CursorPos + size);
        ItemSize(size);
        ItemAdd(bb, 0);
    }
}
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    {
        float y_start = window->DC.CursorPos.y;
        float y_end = window->DC.CursorPos.y + window->DC.CurrLineSize.y;
        ImRect bb(ImVec2(window->DC.CursorPos.x, y_start), ImVec2(window->DC.CursorPos.x + thickness_draw, y_end));
    ...
    {
        float x_start = window->Pos.x;
        float x_end = window->Pos.x + window->Size.x;

        if (g.GroupStack.Size > 0 && g.GroupStack.back().WindowID == window->ID)
            x_start += window->DC.Indent.x;

        if (ImGuiTable* table = g.CurrentTable)
        {
            x_start = table->Columns[table->CurrentColumn].MinX;
            x_end = table->Columns[table->CurrentColumn].MaxX;
        }
        ...
        ImRect bb(ImVec2(x_start, window->DC.CursorPos.y), ImVec2(x_end, window->DC.CursorPos.y + thickness_draw));
...
bool ImGui::SplitterBehavior(const ImRect& bb, ImGuiID id, ImGuiAxis axis, float* size1, float* size2, float min_size1, float min_size2, float hover_extend, float hover_visibility_delay)
{
    ...
    if (held)
    {
        ...
        float max_delta_size1 = ImMax(0.0f, *size1 - min_size1);
        float max_delta_size2 = ImMax(0.0f, *size2 - min_size2);

        mouse_delta = ImClamp(mouse_delta, -max_delta_size1, max_delta_size2);

        if (mouse_delta != 0.0f)
        {
            *size1 += mouse_delta;
            *size2 -= mouse_delta;
            bb_interact.Translate((axis == ImGuiAxis_X) ? ImVec2(mouse_delta, 0.0f) : ImVec2(0.0f, mouse_delta));
            MarkItemEdited(id);
        }
    }

    ImU32 col = GetColorU32(held ? ImGuiCol_SeparatorActive : (hovered && g.HoveredIdTimer >= hover_visibility_delay) ? ImGuiCol_SeparatorHovered : ImGuiCol_Separator);
    window->DrawList->AddRectFilled(bb_interact.Min, bb_interact.Max, col, 0.0f);
...
{
    const ImGuiShrinkWidthItem* a = static_cast<const ImGuiShrinkWidthItem*>(lhs);
    const ImGuiShrinkWidthItem* b = static_cast<const ImGuiShrinkWidthItem*>(rhs);
    return (b->Width != a->Width) ? static_cast<int>(b->Width - a->Width) : (b->Index - a->Index);
}
...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem* items, int count, float width_excess)
{
    ...
    ImQsort(items, static_cast<size_t>(count), sizeof(ImGuiShrinkWidthItem), ShrinkWidthItemComparer);
    ...
    while (width_excess > 0.0f && count_same_width < count)
    {
        ...
        float max_width_removal_per_item = (count_same_width < count && items[count_same_width].Width >= 0.0f) ? (items[0].Width - items[count_same_width].Width) : (items[0].Width - 1.0f);
        if (max_width_removal_per_item <= 0.0f)
            break;

        float width_removal_per_item = ImMin(width_excess / count_same_width, max_width_removal_per_item);
        for (int item_n = 0; item_n < count_same_width; item_n++)
            items[item_n].Width -= width_removal_per_item;

        width_excess -= width_removal_per_item * count_same_width;
    }
    ...
    if (width_excess > 0.0f)
    {
        for (int n = 0; n < count; n++)
        {
            if (items[n].Index < static_cast<int>(width_excess + 0.01f))
                items[n].Width += 1.0f;
        }
    }
}
...
{
    ImGuiContext& g = *GImGui;
    return (items_count > 0) ? ((g.FontSize + g.Style.ItemSpacing.y) * items_count - g.Style.ItemSpacing.y + (g.Style.WindowPadding.y * 2)) : FLT_MAX;
}
...
bool ImGui::DataTypeApplyFromText(const char* buf, ImGuiDataType data_type, void* p_data, const char* format)
{
    ...
    int v32 = 0;
    void* p_result = (type_info->Size >= 4) ? p_data : &v32;
    if (sscanf(buf, format, p_result) < 1)
        return false;
    ...
    if (type_info->Size < 4)
    {
        switch (data_type)
        {
        case ImGuiDataType_S8: *(ImS8*)p_data = (ImS8)ImClamp(v32, (int)IM_S8_MIN, (int)IM_S8_MAX); break;
        case ImGuiDataType_U8: *(ImU8*)p_data = (ImU8)ImClamp(v32, (int)IM_U8_MIN, (int)IM_U8_MAX); break;
        case ImGuiDataType_S16: *(ImS16*)p_data = (ImS16)ImClamp(v32, (int)IM_S16_MIN, (int)IM_S16_MAX); break;
        case ImGuiDataType_U16: *(ImU16*)p_data = (ImU16)ImClamp(v32, (int)IM_U16_MIN, (int)IM_U16_MAX); break;
...
{
    bool modified = false;
    if (v_min && *v < *v_min) { *v = *v_min; modified = true; }
    if (v_max && *v > *v_max) { *v = *v_max; modified = true; }
    return modified;
}
...
{
    static const float min_steps[] = { 1.0f, 0.1f, 0.01f, 0.001f, 0.0001f, 0.00001f, 0.000001f, 0.0000001f, 0.00000001f, 0.000000001f };
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (g.ActiveIdSource == ImGuiInputSource_Mouse && IsMousePosValid() && IsMouseDragPastThreshold(0, g.IO.MouseDragThreshold * DRAG_MOUSE_THRESHOLD_FACTOR))
    {
        ...
        if (g.IO.KeyAlt) adjust_delta *= 0.01f;
        if (g.IO.KeyShift) adjust_delta *= 10.0f;
    }
    else if (g.ActiveIdSource == ImGuiInputSource_Nav)
    {
        const int decimal_precision = is_floating_point ? ImParseFormatPrecision(format, 3) : 0;
        adjust_delta = GetNavInputAmount2d(ImGuiNavDirSourceFlags_Keyboard | ImGuiNavDirSourceFlags_PadDPad, ImGuiNavReadMode_RepeatFast, 0.1f, 10.0f)[axis];
    ...
    TYPE v_cur = *v;
    FLOATTYPE v_old_ref_for_accum_remainder = 0.0f;
...
bool ImGui::DragBehavior(ImGuiID id, ImGuiDataType data_type, void* p_v, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (g.ActiveId != id || (g.LastItemData.InFlags & ImGuiItemFlags_ReadOnly) || (flags & ImGuiSliderFlags_ReadOnly))
        return false;
